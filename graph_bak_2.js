...
var aggregate_data = [],years=[], month_years=[], tiles=[];
...
function color_band(percent)
{
if( percent == -1)
  {
    return -1;
  }
  return  Math.floor(percent*5);
}
...
{
  id = document.getElementById("task_id").selectedOptions[0].value;
  if(id.length > 0 )
  {
    $.ajax({
   url:get_data(id),
   success:function(){
    ...
    $.ajax({ url:update_data(),
	     success:function() {
    		 	d3.selectAll("svg").remove();
   		    make_stacked_graph(chart_data_aggregate());
		      make_kenya_map(tile_data());
		      get_preview_id(id);
          update_map_year_month(0);
	        }});
   }});
}
...
function redraw_chart(type)
{
  ...
  success:function(){
    ...
    $.ajax({ url:update_data(),
      success:function() {
        d3.select("#sidechart").select("svg").remove();
        make_stacked_graph(chart_data_aggregate(type));
...
function update_map_year_month(month_year_selection) {

  month_name = ["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"]
  // adjust the text on the range slider
  date = month_years[month_year_selection];
  map_year_month = date;
  year = date.split("_")[0];
  month = parseInt(date.split("_")[1]);
  d3.select("#map_slider_value").text(month_name[month-1] + ", " + year);
  d3.select("#map_slider").property("value",month_year_selection );

  redraw();
}

d3.select("#map_slider").on("input", function() {
  update_map_year_month(+this.value);
});
...
function tile_data(data_type)
{
  data_type = data_type || "percentage";

  var working_set = raw.map( function(i) { return i.slice(0, 7) });
  working_set = working_set.map( function(i) { return {lat_lon: (i[0] + "_" + i[1]), month_year: (i[2] + "_" + i[3]), remaining: i[4], used: i[5], total: i[6]}; });


  month_years = _.pluck(working_set,"month_year").map(function(i) { return i; }).filter(unique);
  tiles = _.pluck(working_set,"lat_lon").map(function(i) { return i; }).filter(unique);

  // stuff to initialize our hash table
  //temp = Array.apply(null, Array(tiles.length)).map(Number.prototype.valueOf,0);

  month_year_tile_good = {};

  $.each(tiles, function(k,v) {
    $.each(month_years, function(tk,tv) {
       if(month_year_tile_good[v] == null)
         month_year_tile_good[v] = {};

       month_year_tile_good[v][tv] = 0;
    });
  });

//  console.log(month_year_tile_good);

  working_set.forEach( function(d)
  {
    var prev = month_year_tile_good[d.lat_lon][d.month_year];
    var curr = (parseInt(d.total)/(3705*3705));
    month_year_tile_good[d.lat_lon][d.month_year] = (prev > curr) ? prev : curr;

   });
  map_year_month = map_year_month || month_years[0];

  d3.select("#map_slider").attr("max", month_years.length-1);
  return month_year_tile_good;
}
...
function chart_data_aggregate(data_type)
{
  ...
  working_set = working_set.map( function(i) { return {lat_lon: (i[0]+"_"+i[1]),year: i[2], month: i[3], remaining: i[4], used: i[5], total: i[6]}});
...
//  console.log(working_set);
  ...
  working_set.forEach( function(d)
  {
	  ...
	  var key = d.month;
	  console.log(key);
	if(year_month_good[key][parseInt(d.year)-year_min] == null)
		year_month_good[key][parseInt(d.year)-year_min] = {};
	
    if(year_month_good[key][parseInt(d.year)-year_min][d.lat_lon] == null)
	    year_month_good[key][parseInt(d.year)-year_min][d.lat_lon] = init_val;

    if(data_type == "remaining")
      year_month_good[key][parseInt(d.year)-year_min][d.lat_lon] = Math.min(year_month_good[key][parseInt(d.year)-year_min][d.lat_lon],parseInt(d.remaining));
    else if(data_type == "used")
      year_month_good[key][parseInt(d.year)-year_min][d.lat_lon] = year_month_good[key][parseInt(d.year)-year_min][d.lat_lon] + parseInt(d.used);
    else if(data_type == "total")
      year_month_good[key][parseInt(d.year)-year_min][d.lat_lon] = Math.max(year_month_good[key][parseInt(d.year)-year_min][d.lat_lon],parseInt(d.total));
  });
      console.log(year_month_good);
  for(i = 1; i <13; i++)
  {
	...
	{
		var sum = 0;
		for(var k in year_month_good[i][j])
			sum = sum + year_month_good[i][j][k];
		year_month_good[i][j] = sum;
		console.log(sum,i,j);
	}
  }
 ...
 for(j = 0; j < year_count; j++)
{
  for(i = 1; i <13; i++)
  {
		final_set.push({x:((j*12)+i-1) ,y:year_month_good[i][j]});
	}
}
...
var data2 = d3.csv("aggr_tile_timestamp.csv", function(error, dataset) {
  ...
  year_month_good = {};
  year_min = Math.min.apply(null,_.pluck(total,"Year").map(function(i) { return parseInt(i);}));
...
// Set up fill color to red-yellow-green scale
...
var legend = svg.append("g")
	...
	.enter().append("g")
		.attr("transform", function(d, i) { return "translate(20," + (i+7) * 19 + ")"; });
...
d3.json("build/kenya.json", function(error, kenya) {
	...
	svg.append("path")
		.attr("class", "kenya")
                .style("fill", "#ccc")
                .style("stroke", "#fff")
		.datum(topojson.feature(kenya, kenya.objects.country))
		.attr("d", path);

        svg.selectAll(".tile").remove();
	// Draw tiles
	svg.selectAll(".tile")
		.data(topojson.feature(kenya, kenya.objects.tile).features)
		.enter().append("path")
                        .style("fill", function(d) {
			  if(data[d.id] != null)
			  {
                            return color(color_band(data[d.id][map_year_month]));
			  }
                            return "none";

			})
                        .style("stroke", "#fff")
			.attr("class", function(d) { return "tile _" + d.id; })
			.attr("d", path);
...
{
  $.ajax({
     url:ingest_csv_local(),
     success:function(){
       make_stacked_graph(aggregate_data);
       make_kenya_map();
//       make_kenya_map(tile_data());
id = document.getElementById("task_id").selectedOptions[0].value;
